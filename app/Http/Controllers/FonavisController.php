    ...
    public function generateMasivo(Request $request)
{
    ...
    try {
        ...
        $zipper->make($zipPath)->folder('');

        foreach ($projects as $value) {
            $this->generateDocxMulti($value->CerNro, $request->input('idtipo'));
            $zipper->add(storage_path("/fonavis/impresion/" . $ext . substr(rtrim($value->CerNro), 5) . '_' . rtrim($value->CerPosCod) . ".pdf"));
        }
        ...
        \Log::error("Error al generar el ZIP: " . $e->getMessage());
