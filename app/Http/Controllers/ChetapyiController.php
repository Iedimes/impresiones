    ...
    public function generateMasivo(Request $request)
{
    ...
    try {
        ...
        if ($zip->open($zipPath, \ZipArchive::CREATE | \ZipArchive::OVERWRITE) === TRUE) {
            ...
            \Log::info("Iniciando generación de " . count($projects) . " certificados CHETAPYI");
            ...
            foreach ($projects as $index => $value) {
                try {
                    ...
                    $filePath = storage_path("/chetapyi/impresion/" . $fileName);
...
public function generateDocxMulti($id, $tipo)
{
    ...
    $docxPath = storage_path("/chetapyi/impresion/".$CerNro.".docx");
    $qrPath = storage_path("/chetapyi/impresion/".$CerNro.".png");
    $pdfPath = storage_path("/chetapyi/impresion/".$ext.substr(rtrim($postulante->CerNro), 5).'_'.$CerNro.".pdf");
    ...
    if (file_exists($docxPath)) {
        ...
        while (file_exists($docxPath) && $intentos < 3) {
            ...
            $docxPath = storage_path("/chetapyi/impresion/".$CerNro."_".time().".docx");
            $intentos++;
        }
    }
    ...
    if ($tipo == 1) {
        $templates = [
            'AU' => storage_path('/chetapyi/template/autemplate.docx'),
            'PO' => storage_path('/chetapyi/template/potemplate.docx'),
        ];
        ...
        $templatePath = $templates[$postulante->CerMod];
    } else {
        $templatePath = storage_path('/chetapyi/template/chetapyrecibo.docx');
    }
    ...
    // CAMPO1111 - Cabecera según proyecto
    $proyecto = trim($postulante->CerPryCod);
    if ($proyecto == 'K-MR-CT-001') {
        $cabecera = 'LEY N° 6275 – QUE APRUEBA EL MEMORANDO DE ENTENDIMIENTO SOBRE LA COOPERACION BILATERAL ENTRE EL GOBIERNO DE LA REPUBLICA DEL PARAGUAY Y EL GOBIERNO DE LA REPUBLICA DE CHINA (TAIWÁN)';
    } else {
        $cabecera = 'LEY N° 5175/14 – APROBACIÓN DE LA DONACIÓN DE LA REPUBLICA DE CHINA (TAIWÁN)';
    }
    $cabecera = preg_replace('/\s+/', ' ', $cabecera);
    $templateProcessor->setValue('CAMPO1111', $cabecera);
    ...
    $depto = Departamento::find($postulante->CerDptoId);
    $templateProcessor->setValue('CAMPO43', ($postulante->CerDptoId == 0) ? '' : $depto->DptoNom);
    ...
    try {
        ...
        $docxPath = storage_path("/chetapyi/impresion/".$CerNro."_".microtime(true).".docx");
        $templateProcessor->saveAs($docxPath);
    }
